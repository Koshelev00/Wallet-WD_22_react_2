...
function Analysispage() {
  ...
  const handleDayClick = (day) => {
    ...
    else if (!selectedRange[1] && day >= selectedRange[0])
  ...
  return (
    <S.MainBlock>
      {!isMobile && <S.H2>Анализ расходов</S.H2>}
      ...
      <S.ContentContainer>
          ...
          <S.NewExpenseContainer>
            ...
            {showYearView ? (
              ...
              <div ref={calendarRef}>
                <S.DaysOfWeek>
                  {dayNames.map((d, i) => (
                    <S.DayOfWeek key={i}>{d}</S.DayOfWeek>
                  ))}
                </S.DaysOfWeek>
        ...
        {(isMobile ? !showCalendar : true) && (
          <S.ExpensesTableContainer>
            {isMobile && (
              <S.NewExpenseTitle>{'Анализ расходов'}</S.NewExpenseTitle>
            )}
            <S.TableHeader>
              ...
              <ChartComponent
                expenses={transactions
                  .filter((t) => {
                    ...
                    return (
                      transactionDate >= selectedRange[0] &&
